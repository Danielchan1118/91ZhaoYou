...
class jpush {
	...
	function __construct() {
		$this->_masterSecret = "064d899a5fbbbe5a0ba68b73";
		$this->_appkeys = "97109754e6714a64e683893c";
	}
	/**
	 * 模拟post进行url请求
	 * @param string $url
	...
	function send($sendno = 0,$receiver_type = 1, $receiver_value = '', $msg_type = 1, $msg_content = '', $platform = 'android') {
		...
		if (intval($res_arr['errcode'])==0){	
				...
				$data['senduser'] = $this->admin_user;
				$data['add_time'] = time();
				$mess = M('message');
				$res = $mess->add($data);
				$str= "<li>第".$res."条发送".$res_arr['errmsg']."！</li>";
			}else{						
				$str= "<li>第".$res."条发送失败：".$res_arr['errmsg']."</li>";
			}
	    print_r($str);
	}
