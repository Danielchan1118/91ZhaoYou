 ...
 class DelUpdates{
	...
	public function DelUpDate($user_eara = '', $datas = array()){
		...
		if($times > 0){
			...
			$add_list = $add->table('sw_app u')
					  ->join('sw_throw as w on w.app_id = u.id')
	...
	public function UserRecord($datas = array()){
		...
		if($username && $task_type && $times){
			...
			switch($task_type){
				...
				case 'invite':
					$M = M('invitedrecords');
					$record = $M->table('sw_invitedrecords u')
							  ->join('sw_activity_taskrule as w on w.id = u.invited_id')
