 ...
 */
 class AppLogin{
    ...
    public function index($user_eara = array(),$post_data = array()){
		...
		$Mpverinfor = intval($post_data['Mpverinfor']);
        $Mphonemodels = intval($post_data['Mphonemodels']);
        $ProvidersName = intval($post_data['ProvidersName']);
    	...
    	if($mobilephonetype && $mobilewidth && $mobileheight && $Bluetooth_id){
			...
			if($userinfo['username']){
				...
				$User->where("username='".$data['username']."'")->setInc( 'goldcount',$gradelist[0]['getglod'] );
            ...
            if($email && $password){
                $User = M("users");
                $res = $User->field("nickname,goldcount,level")->where("email='".$email."' and password='".$password."'")->find();

                if($res){
                    setcookie("userid",$res['username']);
                    $datas['username'] = $res['username'];
                    $datas['nickname'] = $res['nickname'];
                    $datas['goldcount'] = $res['goldcount'];
					$datas['level'] = 1;
					$datas['levelglod'] = 10000;
					$datas['res'] = 1;
                }else{
