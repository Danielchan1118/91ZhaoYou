(function() {
	...
	function _m() {
		var a = this;
		a.db = UD;
		if(tq && window.openDatabase){ 
			a.db = openDatabase("Pathuku", "1.0", "m", 2097152);
			zK("DB CREATED");
			a.db.zo(function(a) {
	...
	_ga[_] = {
		...
		resize: function(a) {			
			...
			if(b > f * k.dh && c > g * k.dh || c > f * k.dh && b > g * k.dh ){
				d = f; 
				e = g; 
				k.rD = 18; 
				k.RD = 12;
			...
			h != k.rD && (a = a1);
			k.zD = mf((b - k[W_] * k.dh) / 2);//left
			k.ut = mf((c - k[w_] * k.dh) / 2);//top
	...
	_g[_] = {
		init: function() {
		/*
			if(!confirm("进入游戏每关需要10000游币,过关奖励20000游币！是否进入？"))
		   {
		...
		*/
			var a,
			b = this;
			b.UJ = Math.random()*this.levels.length;
			//b.u3();
			c = b.m.read("data");
			...
			if (b.KJ < this.S.KH[0][0]) {
				...
				this.aJ.clear();//
				var tmpLevel = Math.floor(Math.random()* this.levels.length);
				this.sL(this.levels[tmpLevel]);
				return
			}
			
			b.UJ = Math.floor(Math.random()*this.levels.length);
			if(c == UD){
				b.uq(); 
				//b.UE(); 
				//b.u7();
		...
		onclick: function(a, b) {
			window.tmrHandler = this;
			if(typeof window.tmrNotMove!=undefined)clearTimeout(window.tmrNotMove);
			window.tmrNotMove = setTimeout(function(){
					window.tmrHandler.po();
				},5000);
		...
		sL: function(a) {
			window.tmrHandler = this;		
			if("undefined" != typeof window.tmrNotMove){
				clearTimeout(window.tmrNotMove);
			}
			window.tmrNotMove = setTimeout(function(){
					window.tmrHandler.po();
				},5000);
			...
			$.ajax({
				type:"get",
				url:"/Web/GamesApp/gameJson",
				data:"username="+window.username,
				success:function(data){
					if( data.res == -1)
					{	
						var b = new Array;
						window.tmrHandler.UE();
						window.globalD.Y4.zv(window.globalD.s, {kV:60,x:300,y:80}, data.msg, "", b);
						window.setTimeout(function(){ window.Android.ExitGame();},3000);
						return;
					}else if( data.res == 1)
					{					
						var b = new Array;
	...
	_Y4[_] = {
		ZN: function(a) {
			...
			*/
			...
			function() {
				...
				this.g.aJ.clear();//
				var tmpLevel = Math.floor(Math.random()* this.g.levels.length);
				this.g.sL(this.g.levels[tmpLevel]);
			}),
			/*
			g[g[pZ]] = new _Y4I("主菜单",
			function() {
		...
		pq: function(a) {
			...
			*/
			...
			this.zv(a, {kV:60,x:220,y:180}, "游戏结束", "", b)
		},
		KP: function(a) {
			var b,
			c,
		...
		ZV: function(a, b, c) {
			...
			f == UD && (f = function() {
				this.g.aJ.clear(),
				this.g.sL(this.g.levels[Math.floor(Math.random()*this.g.levels.length)])
			}),
			...
			$.ajax({
				...
				success:function(data){
					if(data.res == 1){
						...
						window.tmrHandler.UE();
						window.globalD.Y4.zv(window.globalD.s, {kV:60,x:300,y:80}, data.msg, "", b);
						window.setTimeout(function(){ window.tmrHandler.po();},3000);
						return;
					}else if( data.res == -1){
						var b = new Array;
						window.tmrHandler.UE();
