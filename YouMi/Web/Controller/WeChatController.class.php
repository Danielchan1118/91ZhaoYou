...
class WeChatController extends WebController{
	...
	private function sendCoin($fromUsername,$keyword){
		...
		if($info2){ $is_b = 2; }
		if( 1 == $is_b){
			return "该微信账号已经领取过游币，请勿重复领取，谢谢您的关注！";
		}
		if( 2 == $is_b){
			return "该91找游账号已经领取过游币，请勿重复领取，谢谢您的关注！";
		}
		...
		if($res1){
			...
			$logs['remark'] = "找游账户".$logs['username']."关注微信,赠送".$logs['coin']."游币";
			$scl->add($logs);
			...
			$data['add_time']  = time();
			$data['taskname']  = '关注微信';
			$data['winning']   = "关注微信,赠送".$logs['coin']."游币";
			$task->add($data);
