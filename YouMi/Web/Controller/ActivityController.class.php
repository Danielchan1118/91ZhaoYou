...
class ActivityController extends WebController {
    ...
    public function bigWheel(){
        ...
        if( 3 > $level ){ 
            echo "<script type='text/javascript'>alert('您VIP等级不足，请努力多做任务赚取游币提升VIP等级！')</script>";
            exit;
        }
    ...
    public function dataWheel(){
        ...
        if( 3 > $level ){ 
            ...
            $resStr['str'] = "您VIP等级不足，请努力多做任务赚取游币提升VIP等级！";
            $this->ajaxReturn($resStr);exit;
        }
	...
	public function json_glod(){
		...
		$endtime = strtotime( date("Y-m-d",time() ).'23:59:59');	
			if($data!=''){
				...
				$taskdata['winning'] = "消耗1W游币";
				$taskdata['is_sign'] = 3;
				$datas['returns'] = '很遗憾，您没猜中,扣除1W游币!';
				$task->add($taskdata);
				...
				if($data == 'ok'){
					...
					$taskdata['winning'] = "奖励2W游币";
					$taskdata['is_sign'] = 4;
					$datas['returns'] = '恭喜，您猜中,奖励2W游币!';
