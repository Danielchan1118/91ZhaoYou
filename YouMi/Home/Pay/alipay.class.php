...
class alipay {
	...
	public function BeginPay($arr) {
		...
		require_once $dir . '/YouMi/Home/Pay/alipay/lib/alipay_submit.class.php';
		/**************************请求参数**************************/
		//支付类型
		$payment_type = "1";
		//必填，不能修改
		//服务器异步通知页面路径
		$notify_url = 'http://' . $_SERVER ['HTTP_HOST']."/Home/PayReturn/aliPayAdmin";
		//需http://格式的完整路径，不能加?id=123这类自定义参数
		//页面跳转同步通知页面路径
		//$return_url = "http://demo.31wan.cn/alipay/return_url.php";
		$return_url = 'http://' . $_SERVER ['HTTP_HOST']."/Home/PayReturn/aliPayHome";
	...
	public function CallBack($get = array(), $pay = array()) {
		...
		$dir = $_SERVER ['DOCUMENT_ROOT'];
		require $dir . '/Lib/Pay/alipay/alipay.config.php';
		...
		if($verify_result) {//验证成功
			...
			$trade_status = $_GET['trade_status'];
			 ...
			 if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
				...
				$model->where($map)->setInc('total_money',$pay['pay_money']);//累计消费
				$paymodel = M('pay_ok');
