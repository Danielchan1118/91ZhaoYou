...
class CheckAppController extends HomeController {
	/**
	 * 创建应用 第一步保存
	 */
	public function CreateAppFrist(){
		...
		if($app_name && $app_type && $app_explain && $app_cover && $app_images){
			...
			while($num == 0){
				...
				$appkeynum = $app->where("APP_KEY='".$app_key."'")->count();
				if($appkeynum['tp_count'] == 0){
					$num = 1;
				}
			}
	...
	public function CreateAppThree($app_id = ''){
		...
		if($app_key){
			...
			$app_info = $app->field('APP_KEY,package_name,app_name,app_explain,app_cover,app_images,app_type,stauts,add_time,android_url')->where("id=".$app_id)->find();
